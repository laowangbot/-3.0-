...
class CommentCloneTask:
    ...
    def __init__(self, task_id: str, source_chat_id: str, target_chat_id: str, 
        ...
        # AI文本改写相关
        self.user_id = user_id
        self.ai_rewrite_enabled = config.get('ai_rewrite_enabled', False) if config else False
        self.ai_rewrite_mode = config.get('ai_rewrite_mode', 'auto') if config else 'auto'
        self.ai_rewrite_intensity = config.get('ai_rewrite_intensity', 'medium') if config else 'medium'

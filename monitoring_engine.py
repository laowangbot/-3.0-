...
class RealTimeMonitoringEngine:
    ...
    async def _register_message_handlers(self, task: RealTimeMonitoringTask):
        """注册消息处理器 - 已禁用，改用轮询模式"""
        try:
            # 关键修复：禁用 MessageHandler，使用轮询模式避免占用设备槽位
            logger.info(f"ℹ️ MessageHandler已禁用，使用轮询模式监听 (no_updates)")
            logger.info(f"ℹ️ 轮询检查间隔: {self.check_interval}秒，批处理大小: {self.batch_size}个频道")
            
            # 不再注册 MessageHandler，改用轮询模式
            # 轮询逻辑将在 _poll_messages() 中处理
            ...
            if not hasattr(self, '_polling_started'):
                ...
                logger.info("✅ 轮询监听已启动（no_updates模式）")
        except Exception as e:
    ...
    async def _get_channel_filter_config(self, user_id: str, target_channel: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                import os
                # 使用当前机器人的数据文件
                bot_id = self.config.get('bot_id', 'default_bot')
                channel_data_file = f"data/{bot_id}/channel_data.json"
                os.makedirs(os.path.dirname(channel_data_file), exist_ok=True)
                channel_data_manager = ChannelDataManager(data_file=channel_data_file)

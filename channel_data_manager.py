...
class ChannelDataManager:
    ...
    def remove_all_verified_channels(self) -> int:
        """删除所有已验证的频道，返回删除的数量"""
        try:
            verified_channel_ids = []
            for channel_key, channel_data in list(self.channels_data.items()):
                if channel_data.get('verified', False):
                    verified_channel_ids.append(int(channel_key))
            ...
            if verified_channel_ids:
                self.save_data()
                logger.info(f"✅ 已删除所有已验证的频道: {len(verified_channel_ids)} 个")
            
            return len(verified_channel_ids)
        except Exception as e:
            logger.error(f"删除所有已验证频道失败: {e}")
            return 0

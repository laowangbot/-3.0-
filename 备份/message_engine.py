...
from pyrogram.types import Message, InlineKeyboardMarkup, InlineKeyboardButton, ReplyKeyboardMarkup, ReplyKeyboardRemove, ForceReply
...
class MessageEngine:
        ...
        if effective_config.get('enhanced_filter_enabled', False) and ENHANCED_FILTER_AVAILABLE:
            ...
            try:
                ...
                enhanced_config = {
                    ...
                    "link_based_filtering": True,  # 启用基于链接的过滤模式
                    "ad_keywords": [
                ...
                # 继续使用原始文本，不中断处理流程
            # 即使增强过滤处理完成，如果还启用了常规链接过滤，也要继续处理
        else:
            logger.info("❌ 增强过滤未启用或不可用")
    ...
    async def process_text_with_ai(self, text: str, user_id: Optional[str] = None) -> Tuple[str, bool]:
        """
        使用AI处理文本
        ...
        """
        # 如果没有启用AI或没有AI处理器，则直接返回原文
        if not self.ai_rewriter or not self.config.get('ai_rewrite_enabled', False):
            return text, False
        ...
        # 自动模式下检查额度
        if mode == 'auto' and not (self.ai_rewriter.quota_manager and self.ai_rewriter.quota_manager.has_quota()):
            logger.info("🚫 AI额度不足，使用原文")
            return text, False
        
        # 强制模式下额度用尽则抛出异常
        if mode == 'on' and not (self.ai_rewriter.quota_manager and self.ai_rewriter.quota_manager.has_quota()):
    ...
    def add_additional_buttons(self, original_buttons: Optional[Union[InlineKeyboardMarkup, ReplyKeyboardMarkup, ReplyKeyboardRemove, ForceReply]] = None, config: Optional[Dict[str, Any]] = None) -> Optional[InlineKeyboardMarkup]:
        """添加附加按钮"""
        # 使用指定的配置或全局配置
        effective_config = config or self.config
        ...
        if not additional_buttons:
            # 如果原始按钮不是InlineKeyboardMarkup类型，我们无法合并，只能返回None
            if original_buttons and isinstance(original_buttons, InlineKeyboardMarkup):
        ...
        # 只有当original_buttons是InlineKeyboardMarkup类型时才能合并
        if original_buttons and isinstance(original_buttons, InlineKeyboardMarkup) and original_buttons.inline_keyboard:
    ...
    def filter_buttons(self, buttons: Optional[Union[InlineKeyboardMarkup, ReplyKeyboardMarkup, ReplyKeyboardRemove, ForceReply]], config: Optional[Dict[str, Any]] = None) -> Optional[InlineKeyboardMarkup]:
        """过滤按钮"""
        # 使用指定的配置或全局配置
        effective_config = config or self.config
        
        if not effective_config.get('filter_buttons', False):
            # 如果不是InlineKeyboardMarkup类型，我们无法过滤，直接返回None
            if buttons and not isinstance(buttons, InlineKeyboardMarkup):
                return None
            return buttons
    ...
    def process_message(self, message: Message, channel_config: Optional[Dict[str, Any]] = None, skip_blank_check: bool = False) -> Tuple[Dict[str, Any], bool]:
        ...
        if (effective_config.get('filter_buttons', False) and 
            ...
            self._has_inline_keyboard(original_buttons)):
    ...
    def process_media_group(self, messages: List[Message], channel_config: Optional[Dict[str, Any]] = None) -> Tuple[Dict[str, Any], bool]:
        ...
        # 如果文本被关键字过滤完全移除，整个媒体组也应该被过滤
        if processed_caption == "" and text_modified and len(caption) > 0:
            logger.info("❌ 媒体组文本被关键字过滤，移除整个媒体组")
            return {}, False  # 返回False表示不应该处理这个媒体组
    ...
    def _has_inline_keyboard(self, buttons) -> bool:
        """安全地检查按钮是否包含inline_keyboard属性"""
        if not buttons:
            return False
        if isinstance(buttons, InlineKeyboardMarkup):
            return bool(buttons.inline_keyboard)
        return False

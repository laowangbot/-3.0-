...
DEFAULT_USER_CONFIG = {
    ...
    # 评论区搬运设置（作为过滤设置的一部分）
    "clone_comments": False,  # 是否搬运评论区（默认关闭）
    "comment_clone_limit": 50,  # 每条消息最多搬运多少条评论（0=全部）
    "comment_clone_sort": "chronological",  # 评论排序：chronological（时间顺序）, reverse（倒序）
    "discussion_group_username": {},  # 目标频道的讨论组用户名映射 {target_channel_id: "@username"}
    ...
    # 用户权限设置
    "is_vip": False,  # 是否为会员用户（影响点赞数量等功能）
    "like_count": 1,  # 每条消息的点赞数量（会员用户为3，普通用户为1）
    
    # AI文本改写配置
    'ai_rewrite_enabled': False,           # 默认关闭AI文本改写
    'ai_rewrite_mode': 'auto',             # AI改写模式: on, off, auto
    'ai_rewrite_intensity': 'medium',      # 改写强度: light, medium, heavy
    'ai_tag_handling': 'optimize',         # 标签处理: keep, optimize, replace, extend
    'gemini_api_keys': [],                 # Gemini API密钥列表，支持多个密钥轮询使用
    'current_api_key_index': 0,            # 当前使用的API密钥索引
    
    # 额度提醒配置
    'quota_alert_thresholds': [80, 95],    # 额度提醒阈值(%)
    
    # 多模型配额配置
    'gemini_2_5_flash_lite_quota': 1000,   # gemini-2.5-flash-lite每日调用限制
    'deepai_quota': 10000,                 # DeepAI每日调用限制
    'deepl_write_quota': 5000,             # DeepL Write每日调用限制
    'quillbot_quota': 1000,                # QuillBot每日调用限制
    'rewritify_quota': 100000,             # Rewritify每日调用限制
}
...
def get_config() -> Dict[str, Any]:
    ...
    if not is_render:
        ...
        # 支持从环境变量ENV_FILE指定配置文件
        # 这允许在AWS/多机器人部署时使用不同的配置文件
        ...
        if os.path.exists(env_file):
            ...
            # 如果指定文件不存在，尝试加载默认.env
            if os.path.exists(".env"):
                load_dotenv(".env")
                print(f"⚠️  指定配置文件 {env_file} 不存在，使用默认 .env")
            else:
                print(f"⚠️  配置文件 {env_file} 和 .env 都不存在，使用环境变量或默认配置")
    else:
        print(f"✅ 检测到Render环境，直接使用环境变量")

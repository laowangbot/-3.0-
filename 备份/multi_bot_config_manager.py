...
class MultiBotConfigManager:
    ...
    def _load_from_env_file(self, bot_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            bot_id = os.getenv('BOT_ID', bot_name)
            data_dir = os.getenv('DATA_DIR', f"data/{bot_id}")
            user_session_name = os.getenv('USER_SESSION_NAME', f"user_session_{bot_id}")
            user_api_no_updates = os.getenv('USER_API_NO_UPDATES', 'false').lower() == 'true'
            # 允许在环境中自定义 Bot API session 名称
            session_name = os.getenv('BOT_SESSION_NAME') or os.getenv('SESSION_NAME')
            if not session_name:
            ...
            config = {
                ...
                "user_api_no_updates": 'true' if user_api_no_updates else 'false',

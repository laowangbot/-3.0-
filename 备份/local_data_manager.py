...
class LocalDataManager:
    ...
    async def get_ai_rewrite_config(self, user_id: str) -> Dict[str, Any]:
        """获取AI文本改写配置"""
        try:
            user_config = await self.get_user_config(user_id)
            return {
    ...
    async def set_ai_rewrite_config(self, user_id: str, config: Dict[str, Any]) -> bool:
        """设置AI文本改写配置"""
        try:
            user_config = await self.get_user_config(user_id)
            ...
            # 保存配置
            success = await self.save_user_config(user_id, user_config)
            if success:

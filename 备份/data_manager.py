...
class DataManager:
    ...
    def get_ai_rewrite_config(self, user_id: str) -> Dict[str, Any]:
        """获取AI文本改写配置"""
        try:
            user_config = self.get_user_config(user_id)
            return {
                'enabled': user_config.get('ai_rewrite_enabled', False),
                'mode': user_config.get('ai_rewrite_mode', 'auto'),
                'intensity': user_config.get('ai_rewrite_intensity', 'medium'),
                'tag_handling': user_config.get('ai_tag_handling', 'optimize'),
                'api_keys': user_config.get('gemini_api_keys', []),
                'current_key_index': user_config.get('current_api_key_index', 0)
            }
        except Exception as e:
            logger.error(f"获取AI改写配置失败: {e}")
            return {
                'enabled': False,
                'mode': 'auto',
                'intensity': 'medium',
                'tag_handling': 'optimize',
                'api_keys': [],
                'current_key_index': 0
            }
    
    def set_ai_rewrite_config(self, user_id: str, config: Dict[str, Any]) -> bool:
        """设置AI文本改写配置"""
        try:
            user_config = self.get_user_config(user_id)
            
            # 更新配置
            user_config['ai_rewrite_enabled'] = config.get('enabled', False)
            user_config['ai_rewrite_mode'] = config.get('mode', 'auto')
            user_config['ai_rewrite_intensity'] = config.get('intensity', 'medium')
            user_config['ai_tag_handling'] = config.get('tag_handling', 'optimize')
            if 'api_keys' in config:
                user_config['gemini_api_keys'] = config['api_keys']
            if 'current_key_index' in config:
                user_config['current_api_key_index'] = config['current_key_index']
            
            # 保存配置
            self.save_user_config(user_id, user_config)
            logger.info(f"AI改写配置已更新: user_id={user_id}")
            return True
        except Exception as e:
            logger.error(f"设置AI改写配置失败: {e}")
            return False
    
    def get_quota_alert_thresholds(self, user_id: str) -> list:
        """获取额度提醒阈值"""
        try:
            user_config = self.get_user_config(user_id)
            return user_config.get('quota_alert_thresholds', [80, 95])
        except Exception as e:
            logger.error(f"获取额度提醒阈值失败: {e}")
            return [80, 95]

...
class CloneTask:
    ...
    def __init__(self, task_id: str, source_chat_id: str, target_chat_id: str,
        ...
        self.max_processed_ids = 2000  # 最大保留的消息ID数量（滑动窗口）
        ...
        # 新增：基于消息数量的保存间隔
        self._messages_since_last_save = 0
        self._save_interval_messages = 50  # 每50条消息保存一次
    ...
    def is_duplicate_message(self, message_id: int) -> bool:
        ...
        if message_id in self.processed_message_ids:
            ...
            # 仅在调试模式下记录，避免日志过多
            logger.debug(f"🔄 检测到重复消息: {message_id} (第{self.duplicate_count}次重复)")
            return True
            
        # 额外的安全检查：如果消息ID小于等于恢复点，且是恢复的任务，则可能是重复
        if self.is_resumed and self.resume_from_id and message_id < self.resume_from_id:
            # 但这可能不准确，因为消息ID可能不连续，所以主要依赖processed_message_ids
            # 这里只做记录
            pass
    ...
    def mark_message_processed(self, message_id: int):
        ...
        if len(self.processed_message_ids) > self.max_processed_ids:
            # 移除最小的ID（假设ID是递增的，旧ID不再需要）
            # 为了效率，只有在显著超出时才清理
            if len(self.processed_message_ids) > self.max_processed_ids * 1.2:
                sorted_ids = sorted(list(self.processed_message_ids))
                # 保留最新的 max_processed_ids 个
                self.processed_message_ids = set(sorted_ids[-self.max_processed_ids:])
    
    def save_progress(self, message_id: int):
        """保存当前进度"""
        self.last_processed_message_id = message_id
        self.current_message_id = message_id
        self._messages_since_last_save += 1
        
        # 检查是否满足保存条件：时间间隔 或 消息数量间隔
        current_time = time.time()
        time_due = current_time - self._last_save_time >= self._save_interval
        count_due = self._messages_since_last_save >= self._save_interval_messages
        
        if time_due or count_due:
            # 异步保存到数据库
            asyncio.create_task(self._async_save_progress())
            # 重置计数器（在_async_save_progress中更新时间）
            self._messages_since_last_save = 0
    ...
    async def _async_save_progress(self):
        ...
        try:
            current_time = time.time()
            # 注意：这里不再检查时间间隔，因为在save_progress中已经检查过了
            # 或者是基于消息数量触发的
            ...
            logger.debug(f"任务进度已保存: {self.task_id} (ID: {self.current_message_id})")
    ...
    def prepare_for_resume(self, from_message_id: int):
        ...
        # 初始化已处理集合，避免立即重复处理恢复点之前的消息
        # 注意：这里无法知道之前具体处理了哪些ID，只能依靠resume_from_id
        # 但我们可以假设小于from_message_id的都处理过了（在is_duplicate_message中处理）
        pass
...
class CloningEngine:
    ...
    async def _process_single_message(self, task: CloneTask, message: Message) -> bool:
        ...
        try:
            ...
            # 使用消息引擎处理，传递频道组配置
            processed_result, should_process = await self.message_engine.process_message(message, effective_config)
            ...
            try:
                ...
                while attempt < max_retries:
                    try:
                        ...
                        wait_time = getattr(flood_error, "value", None)
                        if wait_time is None:
                            try:
                                wait_time = int(str(flood_error).split('A wait of ')[1].split(' seconds')[0])
                            except Exception:
                                wait_time = 5
                        logger.warning(f"⚠️ 遇到FloodWait限制，需要等待 {wait_time} 秒 (message_id={message_id})")
                        ...
                        if wait_time > 3600:
                            logger.warning(f"⚠️ FloodWait等待时间过长: {wait_time}秒 ({wait_time/3600:.1f}小时)")
                            if wait_time > 14400:
                                logger.warning("⚠️ 建议暂停任务以避免长时间等待")
                        
                        await asyncio.sleep(wait_time + 1)
                        logger.info("🔄 FloodWait等待完成，继续重试发送媒体消息")
                        # FloodWait不计入尝试次数
                        continue
    ...
    async def _clone_message_comments(self, task: CloneTask, source_message: Message, 
        ...
        try:
            ...
            try:
                ...
                if hasattr(target_chat, 'linked_chat') and target_chat.linked_chat:
            ...
            if False and single_comments:  # 禁用旧的单条媒体处理代码
                ...
                for idx, comment in enumerate(single_comments, 1):
                    try:
                        ...
                        # 处理评论消息（应用过滤规则）
                        processed_result, should_process = await self.message_engine.process_message(comment, config)

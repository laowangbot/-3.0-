...
class CommentCloningIntegration:
    ...
    async def _handle_comment_clone_command(self, message: Message):
        ...
        try:
            ...
            if success:
                await message.reply(f"✅ 评论搬运任务创建成功！\n任务ID: `{task_id}`", parse_mode="Markdown")
            else:
    ...
    async def _handle_comment_status_command(self, message: Message):
        ...
        try:
            ...
            if not args:
                await message.reply("❌ 请提供任务ID\n用法: `/comment_status <task_id>`", parse_mode="Markdown")
                return
    ...
    async def _handle_comment_callback(self, callback_query: CallbackQuery):
        ...
        try:
            ...
            if data.startswith("comment_pause_"):
                task_id = data.replace("comment_pause_", "")
                success = await self.comment_engine.pause_task(task_id)
            ...
            elif data.startswith("comment_cancel_"):
                task_id = data.replace("comment_cancel_", "")
                success = await self.comment_engine.cancel_task(task_id)
            ...
            elif data.startswith("comment_resume_"):
                # 显示恢复预览（从回调触发）
                task_id = data.replace("comment_resume_", "")
                ...
                try:
                    ...
                    if not task:
                        ...
                        await callback_query.message.reply(text, reply_markup=InlineKeyboardMarkup(buttons), parse_mode="Markdown")
                        await callback_query.answer()
                except Exception as e:
                    logger.error(f"处理恢复回调失败: {e}")
                    await callback_query.answer("❌ 处理失败")
            elif data.startswith("comment_confirm_resume_"):
                # 确认恢复：格式 comment_confirm_resume_<task_id>_<resume_from>
                try:
                    payload = data[len("comment_confirm_resume_"):]
                    # task_id 可能包含下划线，右分割一次获取 resume_from
                    if '_' in payload:
                        task_id, resume_from_str = payload.rsplit('_', 1)
                    else:
                        task_id = payload
                        resume_from_str = '0'
                    ...
                    success = await self.comment_engine.resume_task(task_id)
                    if success:
                        await callback_query.answer("✅ 任务已开始恢复")
                        await callback_query.message.reply(f"🔄 任务 `{task_id}` 已开始恢复（从 {resume_from or '上次断点'} 开始）。", parse_mode="Markdown")
                    else:
            ...
            elif data.startswith("comment_preview_"):
                try:
                    task_id = data.replace("comment_preview_", "")
                    task = None
                    if task_id in self.comment_engine.active_tasks:
                        task = self.comment_engine.active_tasks[task_id]
                    else:
